...
int primordial_inflation_find_phi_pivot(
  ...
  if (epsilon > 1.) {
    ...
    case ln_aH_ratio_auto:
      ...
      // KC 6/20/24
      // Error out if this tries to run.  No easy way to do the check that I can see, so just die.
      class_stop(ppm->error_message, "h-less: this code cannot operate as is without an H0 a priori.  Also hardcodes a bunch of other stuff...\n");
